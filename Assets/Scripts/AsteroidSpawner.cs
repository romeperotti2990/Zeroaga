...
public class AsteroidSpawner : MonoBehaviour
{
    ...
    public int playerMaxSpawnAttempts = 200;
    [Range(0.05f, 1f)] public float centerSpawnFraction = 0.5f; // 0..1 fraction of allowed radius to bias spawn toward center
    ...
    void TryRandomizePlayerPosition()
    {
        ...
        if (largeFieldRadius > safeSpawnRadius + 1f)
        {
            maxAllowedRadius = Mathf.Min(worldRadius, largeFieldRadius - safeSpawnRadius - 1f);
        }

        // bias toward center by using a fraction of the allowed radius
        float spawnRadius = Mathf.Max(0.01f, maxAllowedRadius * Mathf.Clamp01(centerSpawnFraction));

        for (int i = 0; i < playerMaxSpawnAttempts; i++)
        {
